...
<div class="col-sm-12 not-imple">
    <div class="p-2">
        ...
        <form class="search" action="{{url('/es_search_implemented')}}" method="get">
            <input class="imput-search" type="text" placeholder=" ชื่อโปรเจค หรือ เรื่อง" name="es_search_implemented">
        ...
        <table class="table table-striped table-hover">
            ...
            <tbody class="">
                ...
                <th class="text-center">
                    @if ($item->name_status == 'ส่งเรื่องร้องเรียน')
                        <span class="bg-primary text-center rounded p-1">{{$item->name_status}}</span>
                    @endif
                </th>
                <th  class="text-center"><a href="{{url('/es_view_implemented',$item->id_complaint)}}"><button class="btn btn-primary btn-sm">ดู</button></a></th>
            </tbody>
            @endforeach
        </table>
