...
<div class="col-sm-12 not-imple">
    <div class="p-2">
        <p class="h-text">จัดการผู้ใช้งาน Employee</p>
        <hr class="m-auto mb-2" style="max-width: 50%;">
        <div class="clearfix mb-2">
            <button type="button" class="btn btn-primary btn-sm float-end me-3" data-bs-toggle="modal" data-bs-target="#addEmployee">เพิ่มข้อมูล</button>
        </div>
        <form class="search" action="{{url('/search_manage_emp')}}" method="get">
            <input class="imput-search" type="text" placeholder=" id or name or email" name="search_manage_emp">
        ...
        <table class="table table-striped table-hover">
            ...
            <tbody class="text-center">
                ...
                <th><a href="{{url('/edit_employee',$item->id)}}"><img src="{{asset('image/edit.gif')}}" style="width:35px;height:35px" alt=""></a></th>
                <th><a onclick="return confirm('คุณต้องการลบข้อมูลหรือไม่ ?')" href="{{url('/delete_employee',$item->id)}}">
...
<!-- เพิ่มผู้ใช้งาน Employee -->
<div class="modal fade" id="addEmployee" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">เพิ่มผู้ใช้งาน (Employee)</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
       <div class="modal-body"> 
            <form id="add_emp" action="add_employee_data" method="post">
            @csrf
        ...
        <div class="modal-footer">
            ...
            <button form="add_emp" type="submit" class="btn btn-success">บันทึก</button>
        </div>
    </div>
  </div>
</div>
