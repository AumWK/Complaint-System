...
<div class="col-sm-12">
    <div class="p-1 ps-2 pe-2">
        ...
        <div class="clearfix pb-2 fs-17 f-weight">
            <a href="{{url('/ep_black_inprogress')}}"><button class="btn btn-secondary btn-sm float-start">กลับ</button></a>
            <div class="float-end">
                @if ($t->name_status == 'กำลังดำเนินการ')
                <button type="button" class="btn btn-sm btn-success" data-bs-toggle="modal" data-bs-target="#finishComplaint">ปิดงาน</button>
                @endif
            </div>
        </div>
...
<!-- Finish job -->
<div class="modal fade" id="finishComplaint" data-bs-backdrop="static" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-sx">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">ปิดงานเรื่องร้องเรียน</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
       <div class="modal-body"> 
            <form id="finish" action="{{url('/ep_finish_inprogress/'.$id_complaint)}}" method="post">
                @csrf
                <!-- เพิ่มข้อความแจ้งเตือน -->
                <input type="hidden" name="message" value="เรื่องร้องเรียนที่ดำเนินการเสร็จสิ้น Project">
                <input type="hidden" name="is_read" value="0">

                <input type="hidden" name="status" value="4">
                <p class="d-inline">วันที่ปิดงาน : <input type="date" name="finish_date" value="<?php echo date("Y-m-d");?>" readonly="readonly"></p>

                <p class="pt-3 fs-17">ระบุหมายเหตุ</p><textarea name="note" cols="40" rows="4"></textarea>
                @error('note')<p class="text-danger">{{$message}}</p>@enderror
            </form>
        </div>
        <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">ออก</button>
            <button form="finish" type="submit" class="btn btn-success">ปิดงาน</button>
        </div>
    </div>
  </div>
</div>
